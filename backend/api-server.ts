/**
 * backend/api-server.ts — Standalone Express API backend
 *
 * Runs independently of Next.js on port 4000 (API_PORT).
 * The Next.js frontend proxies /auth/* and /api/* to this server
 * via the rewrites in frontend/next.config.js (set API_URL=http://localhost:4000).
 *
 * Development (separated):
 *   Terminal 1: npx tsx backend/api-server.ts     (backend, port 4000)
 *   Terminal 2: cd frontend && next dev            (frontend, port 3000)
 *
...
import { connectDb, AssistantSession, FormDraft, User, Application, AuditLog, Officer, TestSlot, FeedbackModel, TestTrack } from './lib/db'
...
app.get('/api/applications/:id/detail', async (req, res) => {
  try {
    await connectDb()
...
app.get('/api/officer/queue', async (req, res) => {
  try {
    await connectDb()
...
app.get('/api/officer/applications/:id/review-data', async (req, res) => {
  try {
    await connectDb()
    const officerId = (req.cookies as Record<string, string>).officerId || 'off-001'
    const app = await Application.findOne({ _id: req.params.id }).lean() as any
