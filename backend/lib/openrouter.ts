...
// If the request takes longer than AI_TIMEOUT_MS (15s), the Promise rejects and the caller falls back to FAQ mode.
function httpsPost(url: string, headers: Record<string, string>, body: string): Promise<{ status: number; body: string }> {
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(AI_TIMEOUT_MS, () => {
      req.destroy(new Error(`OpenRouter timed out after ${AI_TIMEOUT_MS / 1000}s — switching to fallback FAQ`))
    })
    req.write(body)
    req.end()
  })
...
export async function callAssistant(messages: ChatMessage[], lang = 'en'): Promise<ChatMessage> {
  ...
  const body = JSON.stringify({ model, messages: fullMessages })
